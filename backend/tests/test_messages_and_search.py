...
async def test_owner_can_edit_message(make_client, uid):
    alice, _, _, _, _, ch_id = await _two_member_channel(make_client, uid)
    posted = (
        await alice.post(f"/api/channels/{ch_id}/messages", json={"content": "first draft"})
    ).json()
...
async def test_other_users_cannot_edit(make_client, uid):
    alice, bob, _, _, _, ch_id = await _two_member_channel(make_client, uid)
    posted = (
        await alice.post(f"/api/channels/{ch_id}/messages", json={"content": "alice text"})
    ).json()
    msg_id = posted["messageId"]

    r = await bob.patch(f"/api/channels/{ch_id}/messages/{msg_id}", json={"content": "bob attempt"})
    assert r.status_code == 403
...
async def test_edit_unknown_or_wrong_channel(make_client, uid):
    ...
    other_ch = (
        await alice.post(
            f"/api/workspaces/{ws_id}/channels", json={"channelName": "side", "type": "public"}
        )
    ).json()["channelId"]
    posted = (await alice.post(f"/api/channels/{ch_id}/messages", json={"content": "hello"})).json()
    msg_id = posted["messageId"]

    # message exists but not in this channel
    r = await alice.patch(
        f"/api/channels/{other_ch}/messages/{msg_id}", json={"content": "wrong route"}
    )
    assert r.status_code == 404

    # totally unknown message
    r = await alice.patch(f"/api/channels/{ch_id}/messages/999999999", json={"content": "ghost"})
    assert r.status_code == 404
...
async def test_owner_can_delete_message(make_client, uid):
    alice, _, _, _, _, ch_id = await _two_member_channel(make_client, uid)
    posted = (
        await alice.post(f"/api/channels/{ch_id}/messages", json={"content": "regrettable"})
    ).json()
    msg_id = posted["messageId"]
...
async def test_other_users_cannot_delete(make_client, uid):
    alice, bob, _, _, _, ch_id = await _two_member_channel(make_client, uid)
    posted = (await alice.post(f"/api/channels/{ch_id}/messages", json={"content": "stay"})).json()
    msg_id = posted["messageId"]
...
async def test_search_respects_membership(make_client, uid):
    ...
    await alice.post(f"/api/channels/{pub_id}/messages", json={"content": f"{needle} in public"})
    await alice.post(f"/api/channels/{priv_id}/messages", json={"content": f"{needle} in private"})
