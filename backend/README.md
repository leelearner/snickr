...
| `app/core/security.py`  | Password hashing helpers (bcrypt via passlib) |
...
| `API.md`                | Hand-written contract for the frontend (in addition to `/docs`) |
...
- **Auto API docs.** FastAPI generates `/docs` (Swagger UI) and `/redoc` from the type hints, in addition to the hand-written `API.md`. Useful for the demo.
