...
export function ChannelPage() {
  ...
  if (channelQuery.error)
    return (
      <div className="p-6">
        <ErrorState error={channelQuery.error} />
      </div>
    );
  ...
  const headerName =
    channel.type === 'direct'
      ? directPeer
        ? safeDisplayName(directPeer.nickname, directPeer.username)
        : 'Direct message'
      : channel.channelName;

  const HeaderIcon =
    channel.type === 'private' ? Lock : channel.type === 'direct' ? MessageSquare : Hash;
            ...
            {!channel.isMember ? (
              <div className="p-5">
                <ErrorState
                  title="Join required"
                  error={{
                    detail: 'Join this public channel before reading messages.',
                    status: 403,
                  }}
                />
              </div>
